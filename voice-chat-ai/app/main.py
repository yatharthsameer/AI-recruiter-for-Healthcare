...
@app.post("/save_interview_transcript")
async def save_interview_transcript(request: Request):
    """Save interview transcript and metadata to server storage"""
    try:
        data = await request.json()
        ...
        filename = f"Interview_{candidate_name}_{timestamp}.txt"
        filepath = os.path.join(output_dir, filename)

        # Create transcript content
        transcript_content = f"""CAREGIVING INTERVIEW TRANSCRIPT
=====================================

Date: {datetime.now().strftime("%m/%d/%Y, %I:%M:%S %p")}
Session ID: {session_id}
Candidate: {user_data.get('firstName', 'Unknown')} {user_data.get('lastName', 'Candidate')}
        ...
        for entry in transcript:
            ...
            if timestamp:
                try:
                    ...
                    transcript_content += f"[{formatted_time}] {speaker}: {message}\n\n"
                except:
                    # Fallback if timestamp parsing fails
                    transcript_content += f"{speaker}: {message}\n\n"
            else:
                transcript_content += f"{speaker}: {message}\n\n"
...
Generated by CareBot Interview System
Server Timestamp: {datetime.now().isoformat()}
"""
        ...
        # Also save as JSON for potential future processing
        json_filename = f"Interview_{candidate_name}_{timestamp}.json"
        ...
        json_data = {
            ...
            "metadata": {
                ...
                "system": "CareBot Interview System",
                "version": "1.0",
                "audio_filename": f"Interview_{candidate_name}_{timestamp}.webm"  # Expected audio filename
            }
        }
        ...
        logger.error(f"Error saving interview transcript: {e}")
        return {
            "status": "error",
            "message": f"Failed to save transcript: {str(e)}"
        }

@app.post("/save_interview_audio")
async def save_interview_audio(
    audio: UploadFile = File(...),
    userData: str = Form(...),
    sessionId: str = Form(...)
):
    """Save interview audio recording to server storage"""
    try:
        # Parse user data
        import json
        ...
        metadata = {
            ...
            "metadata": {
                "recording_type": "interview_audio",
                "system": "CareBot Interview System",
                "version": "1.0"
            }
        }
